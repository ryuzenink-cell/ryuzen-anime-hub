...
import { validateProductPayload, STORE_CATEGORIES, STORE_BADGES } from "../../../../_utils/store.js";
...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const result = await requireDatabase(env).prepare(`SELECT * FROM store_products WHERE ${conditions.join(" AND ")} ORDER BY sort_order ASC, updated_at DESC, id DESC`).bind(...values).all();
