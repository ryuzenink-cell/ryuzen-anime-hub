...
import { ensureStoreSchema, validateProductPayload } from "../../../../_utils/store.js";
...
export async function onRequestPut({ params, request, env }) {
  try {
    ...
    const result = await db.prepare(`UPDATE store_products SET name=?,category=?,description=?,link_review_status=CASE WHEN affiliate_url<>? THEN 'not_reviewed' ELSE link_review_status END,affiliate_url=?,asin=?,related_title=?,badge=?,image_url=?,image_alt=?,status=?,is_featured=?,sort_order=?,internal_notes=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(p.name,p.category,p.description,p.affiliate_url,p.affiliate_url,p.asin,p.related_title,p.badge,p.image_url,p.image_alt,p.status,p.is_featured,p.sort_order,p.internal_notes,id).run();
