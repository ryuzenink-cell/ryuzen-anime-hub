...
export async function onRequestPost({ params, request, env }) {
  try {
    ...
    const body = await readJson(request).catch(() => ({})); const status = String(body.status || "reviewed");
    if (!["reviewed","needs_check","not_reviewed"].includes(status)) throw new RequestError("Status de revisão inválido.", 400);
    const db = requireDatabase(env); await ensureStoreSchema(db);
    const result = await db.prepare("UPDATE store_products SET link_review_status=?,last_reviewed_at=CASE WHEN ?='reviewed' THEN CURRENT_TIMESTAMP ELSE last_reviewed_at END,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(status,status,id).run();
