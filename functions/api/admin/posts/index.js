...
export async function onRequestPost({ request, env }) {
  try {
    const db = requireDatabase(env); const payload = validatePostPayload(await readJson(request));
    ...
    await replaceTags(db, result.id, payload.tags); await replaceImages(db, result.id, payload.images);
