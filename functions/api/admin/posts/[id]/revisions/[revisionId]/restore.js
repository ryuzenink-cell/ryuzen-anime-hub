...
export async function onRequestPost({ params, request, env }) {
  try {
    ...
    await db.batch([
      db.prepare("INSERT INTO post_revisions (post_id,title,excerpt,content_markdown,revision_note,created_at) VALUES (?,?,?,?,?,CURRENT_TIMESTAMP)")
        .bind(id, current.title, current.excerpt, current.content_html || current.content_markdown || "", "Backup automático antes da restauração"),
      db.prepare("UPDATE posts SET title=?,excerpt=?,content_html=?,content_markdown='',updated_at=CURRENT_TIMESTAMP WHERE id=?")
        .bind(revision.title, revision.excerpt, sanitizeArticleHtml(revision.content_markdown || ""), id),
    ]);
