...
export async function onRequestPut({ params, request, env }) {
  try {
    const db = requireDatabase(env); const id = postId(params);
    ...
    await db.prepare(`UPDATE posts SET title = ?, slug = ?, excerpt = ?, content_markdown = ?, content_html = ?, author_name = ?, category_id = ?, cover_image_url = ?, cover_alt = ?, cover_credit = ?, cover_source_url = ?, social_image_url = ?, seo_title = ?, seo_description = ?, canonical_url = ?, featured = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
      .bind(payload.title, payload.slug, payload.excerpt, payload.content_markdown, payload.content_html, payload.author_name, payload.category_id, payload.cover_image_url, payload.cover_alt, payload.cover_credit, payload.cover_source_url, payload.social_image_url, payload.seo_title, payload.seo_description, payload.canonical_url, payload.featured, id).run();
