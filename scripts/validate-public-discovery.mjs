...
const PUBLIC_VERSION = "20260528-public-discovery-v1";
...
expect(clientApi.includes('const API_BASE_URL = "/api/discovery"'), "O cliente público deve usar a API same-origin /api/discovery.");
expect(!clientApi.includes("api.jikan.moe"), "O navegador não deve consultar a Jikan diretamente.");
expect(serviceWorker.includes('v1.8.0-public-discovery-proxy'), "O service worker deve invalidar caches após a correção da descoberta pública.");
expect(serviceWorker.includes('/assets/js/api.js?v=20260528-public-discovery-v1'), "O service worker deve pré-cachear a versão nova do cliente da API.");
...
expect(ui.includes("icons/icon-192.png?v=20260528-public-discovery-v1"), "A marca no cabeçalho deve reutilizar ícone otimizado e versionado.");
...
for (const page of publicPages) {
  ...
  expect(!html.includes("https://api.jikan.moe"), `${page}: CSP não deve permitir conexão direta desnecessária à Jikan.`);
