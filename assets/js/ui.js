  ...
  header.innerHTML = `
    <header class="site-header">
      <div class="container nav-wrap">
        ...
        <button class="status-button" type="button" data-status-button aria-label="Abrir notificações do sistema" aria-expanded="false">
          <span aria-hidden="true">🔔</span>
          <span class="status-dot hidden" data-status-dot></span>
        </button>
      </div>
    </header>`;
  setActiveNav();
...
let ryuzenCurrentStatus = {
  enabled: false,
  level: "success",
  title: "Tudo funcionando normalmente",
  message: "No momento não há avisos importantes sobre o funcionamento do Ryuzen Anime Hub.",
  updatedAt: "",
  items: []
};
...
function initStatusCenter() {
  ...
  panel.setAttribute("aria-label", "Notificações do sistema");
  ...
  window.addEventListener("ryuzen-api-status", (event) => {
    updateStatusCenter(event.detail);
  });

  updateStatusCenter(ryuzenCurrentStatus);
  loadSiteStatus();

  setInterval(loadSiteStatus, 120000);
}

async function loadSiteStatus() {
  try {
    const response = await fetch(`${dataPath("site-status.json")}?v=${Date.now()}`, {
      cache: "no-store"
    });
    ...
    updateStatusCenter(status);
  } catch {
    updateStatusCenter({
      enabled: true,
      level: "warning",
      title: "Não foi possível verificar os avisos",
      message: "O app não conseguiu carregar a central de status agora. Isso pode ser uma instabilidade temporária.",
...
function updateStatusCenter(status = {}) {
  const button = document.querySelector("[data-status-button]");
  const dot = document.querySelector("[data-status-dot]");
  const panel = document.querySelector("[data-status-panel]");

  if (!button || !dot || !panel) return;

  ryuzenCurrentStatus = {
    enabled: Boolean(status.enabled),
    level: status.level || "info",
    title: status.title || "Aviso do sistema",
    message: status.message || "Nenhuma informação adicional disponível.",
    updatedAt: status.updatedAt || new Date().toISOString(),
    items: Array.isArray(status.items) ? status.items : []
  };

  button.classList.remove("info", "success", "warning", "danger");
  dot.classList.remove("info", "success", "warning", "danger");

  button.classList.add(ryuzenCurrentStatus.level);
  dot.classList.add(ryuzenCurrentStatus.level);
  dot.classList.toggle("hidden", !ryuzenCurrentStatus.enabled);

  const itemsMarkup = ryuzenCurrentStatus.items.length
    ? `<ul>${ryuzenCurrentStatus.items.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>`
    : "";

  panel.innerHTML = `
    <div class="status-panel-header">
      <div>
        <span class="status-label ${escapeHtml(ryuzenCurrentStatus.level)}">${getStatusLevelLabel(ryuzenCurrentStatus.level)}</span>
        <h2>${escapeHtml(ryuzenCurrentStatus.title)}</h2>
      </div>
      <button class="status-close" type="button" data-status-close aria-label="Fechar notificações">×</button>
    </div>

    <p>${escapeHtml(ryuzenCurrentStatus.message)}</p>

    ${itemsMarkup}

    <small>Atualizado em: ${escapeHtml(formatStatusDate(ryuzenCurrentStatus.updatedAt))}</small>
  `;

  panel.querySelector("[data-status-close]")?.addEventListener("click", () => {
    panel.classList.remove("open");
    button.setAttribute("aria-expanded", "false");
  });
}

function getStatusLevelLabel(level) {
  const labels = {
    success: "Normal",
    info: "Informação",
    warning: "Atenção",
    danger: "Instabilidade"
  };

  return labels[level] || "Aviso";
}
...
function notifyApiInstability(message = "A API externa está instável no momento.") {
  window.dispatchEvent(new CustomEvent("ryuzen-api-status", {
    detail: {
      enabled: true,
      level: "danger",
      title: "API de animes instável",
      message,
      updatedAt: new Date().toISOString(),
      items: [
        "Alguns dados podem demorar para carregar.",
        "A busca pode falhar temporariamente.",
